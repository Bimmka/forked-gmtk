...
{
    public class UpdateTaskConcreteRabbitMinAmountSystem : IExecuteSystem
    {
        ...
        public UpdateTaskConcreteRabbitMinAmountSystem(GameContext game)
        {
            _game = game;
        ...
        public void Execute()
        {
            foreach (GameEntity task in _tasks.GetEntities(_buffer))
            {
                ...
                foreach (TaskGoalByRabbitColor goalByColor in task.LevelTaskGoalForConcreteRabbits)
                {
                    ...
                    if (rabbits.Count < goalByColor.MinAmount)
